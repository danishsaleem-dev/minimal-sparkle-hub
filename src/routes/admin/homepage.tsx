  ...
  async function saveSection(section: string, updates: Record<string, unknown>) {
    ...
    try {
      const upserts = Object.entries(updates).map(([key, value]) => ({
        ...
        type: typeof value === "boolean" ? "boolean" : Array.isArray(value) ? "json" : typeof value === "string" && value.startsWith("http") ? "image" : "text",
        position: 0,
      }));
      ...
      setSaved(section);
      setTimeout(() => setSaved((prev) => prev === section ? null : prev), 2000);
  ...
  return (
    <div>
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <SectionCard title="Announcement Banner" icon={<Megaphone size={15} />} expanded={expanded.announcement} onToggle={() => toggle("announcement")}>
          <AnnouncementEditor
        ...
        <SectionCard title="Hero Section" icon={<ImageIcon size={15} />} expanded={expanded.hero} onToggle={() => toggle("hero")}>
          <HeroEditor
            data={data.hero ?? {}}
        ...
        <SectionCard title="Features Strip" icon={<Star size={15} />} expanded={expanded.features} onToggle={() => toggle("features")} hint="4 highlights below hero">
          <FeaturesEditor
            data={data.features ?? {}}
        ...
        <SectionCard title="Brand Story" icon={<BookOpen size={15} />} expanded={expanded.story} onToggle={() => toggle("story")}>
          <StoryEditor
            data={data.story ?? {}}
        ...
        <SectionCard title="Contact Section" icon={<Phone size={15} />} expanded={expanded.contact} onToggle={() => toggle("contact")}>
          <ContactEditor
            data={data.contact ?? {}}
...
function AnnouncementEditor({ data, onSave, saving, saved }: { data: SectionData; onSave: (d: SectionData) => void; saving: boolean; saved: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Input value={text} onChange={(e) => setText(e.target.value)} placeholder="✦ New arrivals every week ✦" className="h-10" />
        <p className="text-xs text-gray-400 mt-1">Use ✦ or · as separators between messages</p>
      </div>
      <SaveButton onClick={() => onSave({ text, enabled })} saving={saving} saved={saved} />
    </div>
  );
}

function HeroEditor({ data, onSave, saving, saved }: { data: SectionData; onSave: (d: SectionData) => void; saving: boolean; saved: boolean }) {
...
function FeaturesEditor({ data, onSave, saving, saved }: { data: SectionData; onSave: (d: SectionData) => void; saving: boolean; saved: boolean }) {
  const raw = data.items;
  const initial: Array<{ label: string; text: string }> =
    Array.isArray(raw) ? (raw as Array<{ label: string; text: string }>) : [];
  const [items, setItems] = useState(initial.length > 0 ? initial : [{ label: "", text: "" }]);
      ...
      {items.map((item, i) => (
        <div key={i} className="flex items-start gap-2 bg-gray-50 rounded-xl p-3">
          ...
          <div className="flex-1 grid grid-cols-2 gap-2">
            <Input value={item.label} onChange={(e) => update(i, "label", e.target.value)} placeholder="Label (e.g. Shipping)" className="h-8 text-sm" />
            <Input value={item.text} onChange={(e) => update(i, "text", e.target.value)} placeholder="Text (e.g. All over Pakistan)" className="h-8 text-sm" />
          </div>
          <button onClick={() => setItems((prev) => prev.filter((_, idx) => idx !== i))} className="p-1.5 text-gray-300 hover:text-red-400 mt-0.5 shrink-0">
            <Trash2 size={14} />
          </button>
        </div>
      ))}
      <Button type="button" variant="outline" size="sm" onClick={() => setItems((prev) => [...prev, { label: "", text: "" }])} className="gap-1.5 border-dashed text-gray-500 hover:text-violet-600">
        <Plus size={13} /> Add Feature
      </Button>
      <SaveButton onClick={() => onSave({ items })} saving={saving} saved={saved} />
    </div>
  );
}

function StoryEditor({ data, onSave, saving, saved }: { data: SectionData; onSave: (d: SectionData) => void; saving: boolean; saved: boolean }) {
  const [title, setTitle] = useState(String(data.title ?? ""));
  const [content, setContent] = useState(String(data.content ?? ""));
  const [image, setImage] = useState<string | null>(data.image ? String(data.image) : null);

  return (
    <div className="space-y-4">
      <div>
        <Label className="text-sm font-medium mb-1.5 block">Story Image</Label>
        <ImageUpload
          value={image}
          onChange={setImage}
          folder="homepage/story"
          aspectRatio="square"
          placeholder="Upload story section image"
          className="max-w-xs"
        />
      </div>
      <div>
      ...
      <div>
        ...
        <Textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Your brand story…" rows={4} className="resize-none" />
      </div>
      <SaveButton onClick={() => onSave({ title, content, image: image ?? "" })} saving={saving} saved={saved} />
    </div>
  );
}

function ContactEditor({ data, onSave, saving, saved }: { data: SectionData; onSave: (d: SectionData) => void; saving: boolean; saved: boolean }) {
  const [title, setTitle] = useState(String(data.title ?? ""));
  const [content, setContent] = useState(String(data.content ?? ""));
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Reach out on WhatsApp or Instagram…" rows={3} className="resize-none" />
      </div>
      <SaveButton onClick={() => onSave({ title, content })} saving={saving} saved={saved} />
    </div>
  );
}

function SectionCard({ title, icon, hint, expanded, onToggle, children }: {
  title: string; icon: React.ReactNode; hint?: string; expanded: boolean; onToggle: () => void; children: React.ReactNode;
}) {
  return (
